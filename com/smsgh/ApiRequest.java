...
public class ApiRequest {
	 ...
	 * Primary constructor.
	 */
	public ApiRequest(String hostname, int port, String protocol,
	 ...
	 */
	private boolean readBytes(ByteArrayOutputStream os, int len)
		throws IOException {
		int nchs = -1;
		byte buf[] = new byte[8192];
		
		for (; len > 0 && (nchs = this.inputStream.read(
		buf, 0, Math.min(len, buf.length))) != -1; len -= nchs)
			os.write(buf, 0, nchs);
	...
	private void open() throws Exception {
		if (this.socket == null) {
			if (this.protocol == "ssl" || this.protocol == "tls") {
				...
				this.socket = sslContext.getSocketFactory()
					.createSocket(this.hostname, this.port);
			} else this.socket = new Socket(this.hostname, this.port);
	...
	public ApiResponse send(String body) throws ApiException {
		...
		if (!this.hasHeader("Host")) {
			this.headers.add(
				new AbstractMap.SimpleEntry("host", this.hostname));
		}
		...
		try {
			...
			this.streamWriter.write(
				buffer.toString(), 0, buffer.length());
			...
			if ((buff = apiResponse.getHeader("content-length"))
				!= null) {
				readBytes(baos, Integer.parseInt(buff));
			}
			
			// Check if the response body is encoded.
			else if ((buff = apiResponse.getHeader("transfer-encoding"))
				!= null) {
				switch (buff) {
					case "chunked":
						chunked :
					...
					default:
						throw new ApiException(
							"Unsupported transfer-encoding: " + buff);
				}
			}
			...
			int offset;
			if ((buff = apiResponse.getHeader("Content-type")) != null
				&& (offset = buff.indexOf('=')) != -1) {
				buff = baos.toString(buff.substring(offset + 1));
			} else buff = baos.toString();
			
			// Set the response body.
			apiResponse.setBody(buff);
			
			// Close the connection if the server says so.
			if ((buff = apiResponse.getHeader("Connection")) != null
				&& buff.toLowerCase().trim().equals("close")) {
				this.close();
			}
